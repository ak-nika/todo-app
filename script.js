...
const renderTasks = () => {
  ...
  tasks.forEach((task) => {
    ...
    const enableEditing = () => {
      ...
      // Replace span with input field for inline editing
      taskDiv.replaceChild(input, taskSpan);

      // Replace iconsDiv with saveButton
      taskDiv.replaceChild(saveButton, iconsDiv);

      // Function to save edited task when pressing Enter
      const saveEdit = () => {
        ...
        if (updatedTask) {
          ...
          // Change items to previous state with updated task
          taskSpan.textContent = updatedTask;
          taskDiv.replaceChild(taskSpan, input);
          taskDiv.replaceChild(iconsDiv, saveButton);
        } else {
          alert("Task cannot be empty...");
        }
      };

      // Activate function on enter key
      ...
      // Activate function on click on save button
      saveButton.addEventListener("click", saveEdit); // Fixed: Removed immediate execution
